...
本项目承接 QuantOS 的「设计公众号发文监控工具」对话，独立维护。仅以真实验证判断可用性。当前已验证目标账号添加、多篇列表翻页、正文获取及报告持久化；单次扫描和正文仍受预算及上游返回限制，完整监控尚未通过。
...
`auth status` 用保存的 Cookie 联网验证，不启动浏览器。读取接口出现认证错误时，每个采集器最多尝试一次自动续期，然后重试读取；列表单独返回 -2041 时先验证书架，不直接续期。可用 `uv run mpwatch auth renew` 主动续期。新 Cookie 必须通过书架验证后才原子保存，续期与扫码共用锁。续期被拒绝时报告 reauth_required 并提示 auth login；网络或限流等错误不直接要求扫码。服务器返回的 Cookie 到期信息写入仓库外 cookie.txt.metadata.json，仅供诊断，不代替真实认证验证。

`--browser chrome` 可切换到已安装的 Chrome；使用 `--browser chromium` 前需执行 `uv run --extra auth playwright install chromium`。扫码功能是可选依赖，普通采集与续期无需 Playwright。
...
- 列表接口曾返回 -2041，后续真实复查已恢复并取得多页文章，原因尚未确定，不能判定接口永久废弃。再次出现 -2041 时，独立验证书架认证有效后将其归类为 list_unavailable，再使用 cover 获取最新一篇。降级结果标记 partial/cover_only，不保证多篇覆盖。
...
2026-09-15 多篇复查：未修改采集代码，现有登录态成功扫描 3 页并新增 55 篇文章记录；本轮 13 篇正文成功、7 篇失败、35 篇未处理，独立进程重读报告一致。由于达到分页预算且正文未全部取得，collection 保持 partial，完整验收仍为 **not_passed**。静态代码检查不计为业务验收通过。

## 下一步

后续需要验证单次群发含多篇的覆盖、历史完整性、失败正文及持续发文发现。当前真实账号和文章内容只保存在仓库外，详见 [验收方案](docs/acceptance.md)。
